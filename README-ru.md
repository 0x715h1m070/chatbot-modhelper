...
<details>
  ...
  - `/add_steam_id <SteamID>` - Добавляет SteamID в чёрный список.
  - `/delete_steam_id <SteamID>` - Удаляет SteamID из чёрного списка.
  - `/list` - Отображает чёрный список SteamID.
  - `/bot_stats` - Отображает список *(ТОП)* модераторов.
  - `/on` - Включает уведомления о новых запросах *(для модератора который ввел эту команду)*.
  - `/off` - Выключает уведомления о новых запросах *(для модератора который ввел эту команду)*.
